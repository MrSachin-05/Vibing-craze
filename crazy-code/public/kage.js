    ...
    (function () {
      ...
      function buildShell() {
        ...
        const sky = new THREE.Mesh(new THREE.PlaneGeometry(360, 190),
          new THREE.MeshBasicMaterial({
            color: hdr(.60, .70, .80), map: tx(texSky()),
        ...
        [[-90, 13, 300, 26, 0], [-63, 9.5, 210, 19, 16]].forEach((r, i) => {
          const m = new THREE.Mesh(new THREE.PlaneGeometry(r[2], r[3]),
            ...
            new THREE.MeshBasicMaterial({
              map: i ? tx(texRidge()) : ridgeMap, transparent: true,
      ...
      function buildTemple() {
        ...
        const grid = new THREE.MeshBasicMaterial({
          map: tx(texShoji()), transparent: true,
      ...
      function buildMoon() {
        const disc = new THREE.Mesh(new THREE.PlaneGeometry(MOON.r * 2, MOON.r * 2),
          ...
          new THREE.MeshBasicMaterial({
            map: tx(texMoon()), color: hdr(3.6, .64, .61),
      ...
      function buildWordmark() {
        ...
        for (const ch of word) {
          ...
          const g = {
            ch: ch, adv: t.width, asc: t.actualBoundingBoxAscent, desc: t.actualBoundingBoxDescent,
        ...
        gl.forEach((g, i) => {
          ...
          const mesh = new THREE.Mesh(new THREE.PlaneGeometry(cw, chh),
            new THREE.MeshBasicMaterial({
              map: tx(c, { aniso: 16 }), transparent: true,
      ...
      function buildAtmosphere() {
        ...
        const emb = new THREE.Points(g, new THREE.ShaderMaterial({
          uniforms: {
            uT: WORLD.uT, uTex: { value: tx(texGlow('rgba(255,190,140,1)', 'rgba(255,120,60,.35)')) },
            uSize: { value: vpH() * .5 }
          },
...
void main () {
  ...
  outColor = vec4(vec3(uDark) * a, a);
      ...
      function createCloth(output, plate, options) {
        ...
        return {
          refresh() { syncSize(); upload(); start(); }, wake: start,
      ...
      const CAM = [
        ...
        { p: [0.0, 9.20, -18.0], t: [0.0, 4.50, -32.0], fov: 45 }, /* 5 archive     */
        { p: [0.0, 10.5, -20.0], t: [0.0, 3.00, -34.0], fov: 46 }  /* 6 footer      */
      ];
      ...
      function wireNav() {
        let last = 0;
        const rail = $('#rail');
        const names = ['The Hidden Gate', 'The Sanmon', 'Still Gardens', 'Sacred Craft', 'Afterlight', 'The Archive', 'Colophon'];
        SECS.forEach((s, i) => {
      ...
      function wireFocus() {
        ...
        $$('[data-chip]').forEach(el => {
          ...
          el.addEventListener('click', () => {
            const targetIdx = +el.dataset.chip + 1;
            if (anchors && anchors[targetIdx] !== undefined) {
              window.scrollTo({ top: anchors[targetIdx], behavior: REDUCE ? 'auto' : 'smooth' });
            }
          });
        });
        $$('[data-les]').forEach(el => {
